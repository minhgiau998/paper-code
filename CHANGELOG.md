...
- **Library Stubs:** Added 20+ missing library templates (Kotlin, Swift, Kubernetes, Terraform, Docker helpers) to prevent generation failures when those stacks are selected.
- **Docker Enhancements:** Included docs for `docker-compose`, multi-stage builds, and container health checks.

### 🔧 Robustness & Developer Experience

- **Self-detection Guard:** Prevent autodetection from picking up the PAPER-CODE repository itself during active development (skip detection when running inside the tool repo).
- **Relative Imports:** Fixed module imports in `src/main.py` so the package entrypoint (`python -m src.main`) works reliably.

### 📦 Release

- **Version bump:** Package version updated to `0.4.0`.
